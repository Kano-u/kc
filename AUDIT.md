...
- 原位置：`src/record.rs`（写入前）、`src/import.rs`（导入前）、`src/history.rs`（读取时再过滤）。
- 问题：同一规则三处实现。读取期过滤是为「filter 添加前就已入库的旧命令」服务，属历史兼容；
  而 README 承诺 filter「把密钥挡在历史数据库之外」，这条只有写入期能做到——
  读取期屏蔽只是不显示，密钥仍在磁盘上。
- 决定（用户选择）：**写入期唯一 + 启动时清理旧行**。
- 已做：
  - 拦截下沉到存储层：`history::upsert` 与 `history::import` 都先判 `config.filtered`，
    调用方（`record.rs`、`import.rs`）不再自己判断——**没有任何写入路径能绕过 filter**。
  - 新增 `history::purge_filtered`：启动时（`app::run_main`）把 filter 生效前入库的命中行
    物理删除。唯一调用点，`import` 不重复清（它只写不命中行）。
  - `history::load` 删掉读取期过滤与内存截断，改为 SQL `ORDER BY at DESC LIMIT ?1` 后反转。
- 结果：`config.filtered` 的调用点只剩 `history.rs` 一处文件；命令数超限时不再把整表读进内存。
